...
Salary-specific deterministic feature engineering stage.

Input:
    artifacts/feature_store/latest/feature_store.parquet

Output:
    artifacts/salary_feature_store/latest/
        salary_modeling_dataset.parquet
        salary_feature_metadata.json
        salary_feature_report.json
        schema_fingerprint.json

Optional archive:
    artifacts/salary_feature_store/archive/<dataset_id>/

IMPORTANT
---------
This stage performs only deterministic transformations.
...
Those operations belong after train/validation/test splitting to prevent
data leakage.
"""
...
class SalaryFeatureEngineering:
    ...
    def initiate_salary_feature_engineering(
        ...
        logging.info(
            "=== Salary Feature Engineering started ==="
        )
        ...
        try:
            ...
            self._validate_predictor_configuration(
                present_predictors
            )
            ...
            if final_row_count == 0:
                ...
                raise ValueError(
                    "Salary feature engineering produced "
                    "0 valid salary rows."
                )
            ...
            posting_group_id = (
                self._build_posting_group_id(df)
            )
            ...
            annual_stats = self._describe(
                out[cfg.target_annual_col]
            )

            log_stats = self._describe(
                out[cfg.target_log_col]
            )

            source_counts = (
                out[cfg.target_source_col]
            ...
            summary = SalaryFeatureEngineeringSummary(
                ...
                salary_candidate_count=(
                    funnel_counts[
                        "salary_candidate_count"
                    ]
                ),

                valid_usd_salary_count=(
                    funnel_counts[
                        "valid_usd_salary_count"
                    ]
                ),
            ...
            logging.exception(
                "Salary Feature Engineering failed."
            )
    ...
    def _load_feature_store(
        ...
        if not path.exists():
            ...
            raise FileNotFoundError(
                f"Common feature store not found: {path}"
            )
    ...
    def _validate_required_columns(
        ...
        if missing:
            ...
            raise ValueError(
                "Required target-construction columns "
                f"missing: {missing}"
            )
    ...
    def _resolve_candidate_predictors(
        ...
        configured = (
            self.config.candidate_predictor_columns
        )
    ...
    def _validate_predictor_configuration(
        ...
        leakage = (
            set(predictors)
            & set(self.config.leakage_columns)
        )
    ...
    def _construct_target(
        ...
        raw_salary.loc[has_range] = (
            min_salary.loc[has_range]
            + max_salary.loc[has_range]
        ) / 2.0

        raw_salary.loc[has_median] = (
            med_salary.loc[has_median]
        )
        ...
        target_source.loc[has_range] = (
            cfg.source_range_midpoint_label
        )

        target_source.loc[has_median] = (
            cfg.source_median_salary_label
        )

        # ------------------------------------------------------------
        # Salary candidate accounting
        # ------------------------------------------------------------

        salary_candidate_mask = (
            raw_salary.notna()
        )

        salary_candidate_count = int(
            salary_candidate_mask.sum()
        )

        rows_removed_missing_salary = int(
            (~any_salary_present).sum()
        )

        # Invalid salary information exists but could not produce
        # a valid midpoint/median. The current entity does not expose
        # this as a separate field, so log it instead.
        invalid_salary_mask = (
            any_salary_present
            & ~salary_candidate_mask
        )
        ...
        invalid_salary_count = int(
            invalid_salary_mask.sum()
        )

        if invalid_salary_count:

            logging.warning(
                "%d rows contained salary information but "
                "could not produce a valid salary candidate.",
        ...
        multiplier = (
            pay_period
            .map(cfg.pay_period_multipliers)
            .astype("float64")
        )
        ...
        target_annual.loc[valid_period_mask] = (
            raw_salary.loc[valid_period_mask]
            * multiplier.loc[valid_period_mask]
        )
        ...
        target_annual = (
            target_annual.where(final_mask)
        )

        target_source = (
            target_source.where(final_mask)
        )
    ...
    def _build_numeric_features(
        ...
        def safe_log1p(
            ...
            if column not in df.columns:
                ...
                logging.warning(
                    "%s not found. Generated log feature "
                    "will contain NaN.",
                    column,
                )
        ...
        log_employee = safe_log1p(
            cfg.company_employee_count_col
        )

        log_follower = safe_log1p(
            cfg.company_follower_count_col
        )
    ...
    def _build_posting_group_id(
        ...
        for column in (
            cfg.posting_group_source_columns
        ):
            ...
            if column in df.columns:
                ...
                logging.warning(
                    "posting_group_id source column "
                    "%s missing; using 'unknown'.",
                    column,
                )
        ...
        if not parts:
            ...
            raise ValueError(
                "No posting-group source columns configured."
            )
    ...
    def _assemble_output_frame(
        ...
        out[cfg.target_annual_col] = (
            target_annual.loc[selected_index]
        )

        out[cfg.target_log_col] = (
            target_log.loc[selected_index]
        )

        out[cfg.target_source_col] = (
            target_source.loc[selected_index]
        )
        ...
        out[
            cfg.log_company_employee_count_col
        ] = log_employee.loc[selected_index]

        out[
            cfg.log_company_follower_count_col
        ] = log_follower.loc[selected_index]
    ...
    def _run_integrity_checks(
        ...
        if out.empty:
            ...
            raise ValueError(
                ...
                "output dataset contains 0 rows."
            )
        ...
        if out[
            ...
            raise ValueError(
                "Integrity check failed: "
                "annual salary target contains NaN."
            )
        ...
        if out[
            ...
            raise ValueError(
                "Integrity check failed: "
                "log salary target contains NaN."
            )
        ...
        if not valid_bounds.all():
            ...
            raise ValueError(
                "Integrity check failed: "
                "salary targets outside configured bounds."
            )
        ...
        if len(numeric_columns):
            ...
            if np.isinf(
                ...
                raise ValueError(
                    "Integrity check failed: "
                    "numeric features contain infinity."
                )
        ...
        if (
            ...
            raise ValueError(
                "Integrity check failed: "
                "posting_group_id contains nulls."
            )
        ...
        if "index" in out.columns:
            ...
            raise ValueError(
                "Integrity check failed: "
                "accidental index column persisted."
            )
    ...
    def _write_artifacts(
        ...
        temp_dataset_path = (
            cfg.latest_dir
            / "salary_modeling_dataset.tmp.parquet"
        )
        ...
        return SalaryFeatureEngineeringResult(
            ...
            archive_schema_fingerprint_path=(
                archive_schema_fp_path
            ),
