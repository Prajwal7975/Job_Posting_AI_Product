...
class DataCleaning:
    ...
    def _convert_dtypes(
        ...
        target_dtypes = (table_schema.canonical_dtypes
    ...
    def _remove_orphans(
        ...
        for fk_col, (parent_table, parent_key) in rule.foreign_keys.items():
            ...
            logging.info(f"{fk_col} dtype: {df[fk_col].dtype}")
            logging.info(f"{parent_key} dtype: {parent_df[parent_key].dtype}")
            
            logging.info(df[fk_col].head())
            logging.info(parent_df[parent_key].head())
            logging.info(f"Unique child keys: {df[fk_col].nunique()}")
            logging.info(f"Unique parent keys: {parent_df[parent_key].nunique()}")

            valid_keys = set(parent_df[parent_key].dropna().unique())
            
            matches = df[fk_col].isin(valid_keys).sum()
            
            logging.info( f"[{fk_col}] Matching rows: {matches}/{len(df)}")
